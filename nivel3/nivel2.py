...
def dibujar_resultados_finales(surf, estadisticas):
    ...
    if (tiempo_actual // 500) % 2 == 0:  # Parpadeo cada 500ms
        mensaje = FUENTE_ARCADA.render("PRESIONA ESPACIO para continuar al NIVEL 3 o ESC para menú", True, BLANCO)
...
def juego():
    ...
    titulo_inicio = "NIVEL 2: LA FORTALEZA DEL CAOS"
    subtitulo_inicio = "MISIÓN: DERROTAR AL GUARDIÁN DIMENSIONAL"
    
    historia_inicio = (
        "Después de escapar de la Nebulosa Oscura, tu nave detecta una anomalía dimensional "
        "en el sistema estelar Zeta-7. Una fortaleza flotante, conocida como 'El Ojo del Caos', "
        "emite energías distorsionadoras que amenazan con desestabilizar toda la galaxia. "
        "Te adentras en sus pasillos llenos de peligros y trampas mortales, con un solo objetivo: "
        "destruir al Guardián Dimensional que mantiene activo el portal interdimensional. "
        "Prepárate para la batalla más difícil hasta ahora."
    )
    
    # Textos para la historia FINAL (puedes modificar estos textos según tu historia)
    titulo_final = "MISIÓN CUMPLIDA"
    subtitulo_final = "EL PORTAL SE CIERRA"
    
    historia_final = (
        "Con el Guardián Dimensional derrotado, el portal interdimensional comienza a colapsar. "
        "La energía caótica se dispersa por el espacio, restaurando el equilibrio dimensional. "
        "La fortaleza 'Ojo del Caos' se desintegra lentamente mientras escapas a toda velocidad. "
        "Sin embargo, tus sensores detectan una señal aún más poderosa en el sector Omega-9. "
        "Parece que esta era solo la primera línea de defensa... La verdadera amenaza aún espera."
    )
                    ...
                    if event.key == pygame.K_SPACE:
                        ...
                        try:
                            ...
                            if os.path.exists("nivel3"):
                                ...
                                # Aquí iría la lógica para cargar el nivel 3
                                # Por ahora solo reiniciamos el nivel 2
                                puntaje, estado = reiniciar_nivel(completo=True)
                                tiempo_inicio_conteo = pygame.time.get_ticks()
                                tiempo_inicio_nivel = pygame.time.get_ticks()
                                enemigos_eliminados = 0
                                ...
                                print("Nivel 3 no disponible aún")
                                puntaje, estado = reiniciar_nivel(completo=True)
                                tiempo_inicio_conteo = pygame.time.get_ticks()
                                tiempo_inicio_nivel = pygame.time.get_ticks()
                                enemigos_eliminados = 0
